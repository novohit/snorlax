...
//    }
    ...
    @Bean(name = "deduplicate")
    public DefaultRedisScript<ArrayList> lockScript() {
        DefaultRedisScript<ArrayList> limitScript = new DefaultRedisScript<>();
        // 设置脚本返回的类型
        limitScript.setResultType(ArrayList.class);
        // 设置脚本位置 根目录从resource开始
        limitScript.setScriptSource(new ResourceScriptSource(new ClassPathResource("lua/deduplicate.lua")));
        return limitScript;
    }
}
